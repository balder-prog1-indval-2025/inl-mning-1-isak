...
async function fetchImages(...paths) {
  return await Promise.all(paths.map(
...
function str(value) {
  ...
  } else if (typeof value == "object" && (value != null && Object.getPrototypeOf(value) === Object.prototype && value.toString == Object.prototype.toString)) {
...
class Button {
  constructor(text2, x = 0, y = 0, width, height, color = "lightgrey", fontSize = 16, textColor = "black") {
    this.text = text2;
    this.color = color;
    this.fontSize = fontSize;
    this.textColor = textColor;
    width = width ?? text2.length * fontSize;
...
class Sprite extends Hitbox {
  ...
  update() {
    ...
    if (this.remainingTime < 0) {
      ...
      this.remainingTime += 1e3 / this._framesPerSecond;
    }
    this.draw();
  }
  draw() {
...
function array2D(rows, columns, value) {
  return Array.from({ length: rows }, (_, i) => Array.from({ length: columns }, (_2, j) => typeof value == "function" ? value(i, j) : value));
}
let _audioContext;
const _audioList = [];
function beep(frequency = 800, msDuration = 200, volume = 1) {
  ...
  return new Promise((resolve) => {
    ...
    audioItem[0].stop(_audioContext.currentTime + msDuration / 1e3);
  });
}
